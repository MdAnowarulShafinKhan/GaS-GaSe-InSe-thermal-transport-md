#!/usr/bin/env python3
# =============================================================================
#  Monolayer InSe - harmonic phonon dispersion + total/projected phonon DOS
# =============================================================================
#
#  Method:
#    1. Build the four-atom X-M-M-X primitive cell.
#    2. Relax the in-plane cell and atomic coordinates with the InSe Tersoff EIAP.
...
#                    a0       c        h       d_M-M    d_M-X
#    DFT            4.095   25.000    5.379    2.815    2.689
#    Tersoff EIAP   4.341   24.251    4.816    2.537    2.753
#
...
#      python inse_phonon_dispersion_pdos.py
# =============================================================================
...
M_ELEMENT = "In"
X_ELEMENT = "Se"
POTENTIAL = "InSe.tersoff"
ELEMENTS = [M_ELEMENT, X_ELEMENT]  # LAMMPS type 1 = M, type 2 = X
MASSES = {1: 114.818000, 2: 78.96000000}  # atomic mass units

# EIAP equilibrium geometry from Supporting Information Table S8.
A_GUESS = 4.341000       # in-plane lattice constant a0 (A)
M_M_GUESS = 2.537000    # vertical metal-metal distance d_M-M (A)
THICK_GUESS = 4.816000   # outer X-X monolayer thickness h (A)
D_MX_REFERENCE = 2.753000  # nearest metal-chalcogen bond d_M-X (A)

# The paper reports c = 24.251 A for its relaxed EIAP cell. Here 25 A is used
...
def build_primitive():
    """Build the four-atom Se-In-In-Se primitive cell."""
    a = A_GUESS
    z_m = M_M_GUESS / 2.0
    z_x = THICK_GUESS / 2.0
...
def compute_dos(phonon):
    ...
    # Match atoms to the two user-specified masses. This works for GaSe and InSe;
    # unlike a simple >50 amu threshold, it does not misclassify Se.
